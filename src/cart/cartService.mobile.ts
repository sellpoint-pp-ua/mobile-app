...
export async function addToCart(productId: string, pcs = 1): Promise<void> {
  ...
  for (const u of bothPaths("Cart/AddToCart")) {
    addCandidates.push({
      url: u,
      init: { method: "POST", headers: HAuth(token), body: fd1 as any },
      describe: `POST multipart ${u} (ProductId,Pcs)`,
    });
    addCandidates.push({
      url: u,
      init: { method: "POST", headers: HAuth(token), body: fd2 as any },
      describe: `POST multipart ${u} (productId,pcs)`,
    });
  }
  ...
  for (const u of bothPaths("Cart/ChangeCartPcs")) {
    ...
    changeByProductId.push({
      url: `${u}?${q}`,
      init: { method: "PUT", headers: HAuth(token) },
      describe: `PUT ${u}?${q} (id=productId)`,
    });
  }
  ...
  if (!cartItemId) {
    throw new Error("Не вдалося додати у кошик (позиція не знайдена в кошику)");
  }
